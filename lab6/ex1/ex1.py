def verifica_a(nr_pompare):
    ...
    for lungime_y in range(1, nr_pompare + 1):
        ...
        cuvant_pompat_jos = parte_x + parte_z
        numar_0 = cuvant_pompat_jos.split('1')[0].count('0')
        numar_1 = len(cuvant_pompat_jos) - numar_0
...
def verifica_b(nr_pompare):
    ...
    for lungime_y in range(1, nr_pompare + 1):
        ...
        cuvant_pompat_sus = parte_x + parte_y + parte_y + parte_z
        numar_a = cuvant_pompat_sus.split('b')[0].count('a')
        numar_b = len(cuvant_pompat_sus) - numar_a
    ...
    for start_y in range(nr_pompare):
        for lungime_y in range(1, nr_pompare + 1):
            if start_y + lungime_y <= len(cuvant_ex):
                ...
                cuvant_pompat = parte_x + parte_y + parte_y + parte_z
                print(f"Pompare (y='{parte_y}'): {cuvant_pompat}")

                numar_a = cuvant_pompat.split('b')[0].count('a')
                numar_b = cuvant_pompat[numar_a:].split('c')[0].count('b')
                numar_c = len(cuvant_pompat) - numar_a - numar_b
...
if __name__ == "__main__":
    nr_pompare = int(input("Introdu numar pompare: "))
    verifica_a(nr_pompare)
    print()
    verifica_b(nr_pompare)
    print()
    verifica_c(nr_pompare)
